...
from nets.devformer import DevFormer as AttentionModel
...
def run(opts):
    ...
    if opts.wandb != 'disabled':
        ...
        wandb.init(project='symrd_hardware', 
                    entity='hyeonah_kim',
                    group=opts.problem + '-' + opts.method,
                    name=run_name,
                config=opts)
    ...
    if opts.baseline == 'rollout':
        # milestones = [7, 10]  # epoch
        milestones = [5, 8]  # epoch_size 600 val_size 300
        # milestones = [100]  # epoch
    else:
        # milestones = [300]
        # milestones = [20, 30]
        milestones = [13, 20]  # epoch_size 600 val_size 300
