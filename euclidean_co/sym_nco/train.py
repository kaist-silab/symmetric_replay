...
def train_epoch(model, optimizer, baseline, lr_scheduler, epoch, val_dataset, problem, tb_logger, opts):
    ...
    cum_samples = epoch * (opts.epoch_size * opts.N_aug)
    ...
    for batch_id, batch in enumerate(tqdm(training_dataloader, disable=opts.no_progress_bar)):
        ...
        if opts.problem == 'cvrp':
            cum_samples += batch['depot'].shape[0] * opts.N_aug
        else:
            cum_samples += batch.shape[0] * opts.N_aug
        ###########################################################################
...
def train_batch(
    ...
    if opts.il_coefficient > 0.:
        cost, log_likelihood, pi = model(x_aug, return_pi=True)
