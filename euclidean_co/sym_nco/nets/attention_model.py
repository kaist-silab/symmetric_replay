...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False, return_proj=False, action=None, sub_len=0):
        """
        ...
        if sub_len > 0:
            sub_ll_1, sub_ll_2 = self._calc_log_likelihood_sub(_log_p, pi, mask, pi.shape[1] - sub_len)
            return cost, ll, sub_ll_1, sub_ll_2, pi
    ...
    def _calc_log_likelihood_sub(self, _log_p, a, mask, sub_len):
        """ SymRD """
        # Get log_p corresponding to selected actions
        log_p = _log_p.gather(2, a.unsqueeze(-1)).squeeze(-1)
        ...
        # Calculate log_likelihood
        return log_p[:, :sub_len].sum(1), log_p[:, sub_len:].sum(1) 
    ...
    def _inner(self, input, embeddings, action=None):
        ...
        ############################### [SymRD] ###################################
        log_ps = torch.stack(outputs, 1)
        actions = torch.stack(sequences, 1)
        
        if self.is_vrp:
            pi_padding = torch.zeros(batch_size, 2 * log_p.shape[-1] - actions.shape[1]).to(actions.device)
            actions = torch.cat([actions, pi_padding.long()], dim=1)

            log_ps_padding = -999 * torch.ones(batch_size, 2 * log_p.shape[-1] - log_ps.shape[1], log_p.shape[-1]).to(log_p.device)
            log_ps_padding[:, :, 0] = 0
            log_ps = torch.cat([log_ps, log_ps_padding], dim=1)
        
        return log_ps, actions
    ...
    def _get_log_p(self, fixed, state, normalize=True):
        ...
        log_p = torch.clamp(log_p, min=-999)  # [SymRD] added
