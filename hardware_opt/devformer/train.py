...
def train_epoch(model, optimizer, baseline, lr_scheduler, epoch, val_dataset, problem, tb_logger, opts):
    ...
    if opts.baseline == 'rollout':
        ...
        # if epoch > 0:
        #     cum_samples += opts.epoch_size  # greedy rollout
    else:
    ...
    if opts.wandb == 'disabled':
        ...
        wandb_log_dict = {'val_avg_reward': (-1)*avg_reward,
...
def train_batch(
    ...
    if opts.distil_mask == 'bl' and opts.baseline != 'no_baseline':
        ...
        distil_mask = (cost < cost.mean()).view(-1)
    ...
    wandb_log_dict = {'avg_reward': (-1)*cost.mean().item(),
                    'cum_samples': cum_samples,
...
def train_batch_ppo(
    ...
    if opts.il_coefficient > 0 and (step + 1) % opts.distil_every == 0:
        ...
        if opts.clip_distil_ll:
            distill_loss = distill_model(model, optimizer, opts, x[distil_mask], pi[distil_mask], ll_clip=ratio.mean().item())
        else:
            distill_loss = distill_model(model, optimizer, opts, x[distil_mask], pi[distil_mask])
    else:
        distill_loss = 0
...
def distill_model(model, optimizer, opts, x, pi=None, ll_clip=0.):
    ...
    for _ in range(opts.distil_loop):
        ...
        _, log_likelihood_IL = model(new_x, action=action)
