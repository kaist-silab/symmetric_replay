...
def symmetric_action(action, opts):
    ...
    elif opts.problem == 'dpp' or 'dpp_hbm':
        perm = np.random.permutation(action.shape[1])
        sym_action = action[:,perm]        
...
def rollout_for_self_distillation(model, opts, x):
    ...
    with torch.no_grad():
        _, log_likelihood, pi = model(x, return_pi=True, reward_free=True)
