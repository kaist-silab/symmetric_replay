...
def generate_decap_data(decap_x = 15, decap_y = 20):
    
 
    probing_port = range(0,decap_x*decap_y)

    dpp = np.zeros((len(probing_port),decap_x*decap_y, 3))
    ...
    for i in range(len(probing_port)):
        ...
        for x in range(decap_x):
            for y in range(decap_y):
                ...
                dpp[i,x*decap_y+y, 0] = x/normalization
                dpp[i,x*decap_y+y,1] = y/normalization

                if np.random.uniform(0,1)>0.9 and num_restriction < 15:                        
                    dpp[i,x*decap_y+y,2] = 1
                    num_restriction += 1

                if x*decap_y+y ==probing_port[i]:           
                    dpp[i,x*decap_y+y,2] = 2
